...
function removeLastOccurrences(str, value) {
  ...
  if (find === -1) return str;
  return str.slice(0, find) + str.slice(find + value.length);
}
...
function findLongestWord(sentence) {
  ...
  const arrLength = arr.map((words) => words.length);
    ...
    .map((char) => {
      if (char.charCodeAt(0) >= 65 && char.charCodeAt(0) <= 90) {
        if (char.charCodeAt(0) + 13 > 90) {
          return String.fromCharCode(64 + char.charCodeAt(0) + 13 - 90);
        }
        return String.fromCharCode(char.charCodeAt(0) + 13);
      }
      if (char.charCodeAt(0) >= 97 && char.charCodeAt(0) <= 122)
        if (char.charCodeAt(0) + 13 > 122) {
          return String.fromCharCode(96 + char.charCodeAt(0) + 13 - 122);
        } else {
          return String.fromCharCode(char.charCodeAt(0) + 13);
        }
      return char;
